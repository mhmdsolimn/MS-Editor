...
let result , browser , Editor , EditorCss , EditorJs , autoSave , htmlBtn , cssBtn , jsBtn , introBtn , introLoadDiv
    , introLoadVal , introLoad , intro , saveBtn;
...
const items1 = ["@nav" , "@h1" , "@h2" , "@h3" , "@h4" , "@h5" , "@h6" , "@p" , "@div" , "@title" , "@header" , "@head" , "@body" , "@style" ,
    "@html" , "@script" , "@i" , "@span" , "@sup" , "@sub" , "@textarea" , "@footer" , "@ul" , "@li" , "@ol" , "@dl" , "@dt" , "@dd" , "@center" , "@button" , "@form" ,
    "@table" , "@tr" , "@td" , "@thead" , "@tbody" , "@tfoot" , "@caption" , "@main" , "@select" , "@option"];

const items2 = ["@br" , "@hr" , "@meta"];

const Attrebutes = ["$class" , "$id" , "$src" , "$width" , "$height" , "$href" , "$type" , "$style" , "$value" , "$onclick" , "$ondblclick" ,
    "$oncontextmenu" , "$onkeyup" , "$onkeydown" , "$onkeypress" , "$rel" , "$charset" , "$lang" , "$dir"]
...
Editor.addEventListener("keyup" , function(e){
    if (Editor.value.includes("@audio") && e.key == "Enter") {
        ...
        Editor.value = Editor.value.replace("@link" , '<link rel="stylesheet" href="">')
